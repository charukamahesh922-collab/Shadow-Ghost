...
<body>
    <div class="container">
        ...
        {self._generate_emails_html(data.get('emails', []))}
        {self._generate_sensitive_html(data.get('sensitive_files', []))}
        {self._generate_links_html(data.get('links', []))}
    ...
    def _generate_emails_html(self, emails):
        """Generate emails section for HTML report"""
        if not emails:
            return ""
    ...
    def _generate_sensitive_html(self, sensitive_files):
        """Generate sensitive files section for HTML report"""
        ...
        for file_info in sensitive_files:
            html += f"<tr><td>{file_info.get('file', 'Unknown')}</td><td>{file_info.get('url', 'N/A')}</td></tr>"
        html += """
            </table>
        </div>
        """
    ...
    def _generate_links_html(self, links):
        """Generate links section for HTML report"""
        if not links:
            return ""
        ...
        if len(links) > 50:
            html += f"<li>... and {len(links) - 50} more</li>"
        html += """
            </ul>
        </div>
        """
...
"""
        ...
        if data.get('links'):
            md += "\n## Links Found\n"
            for link in data.get('links', [])[:50]:
                md += f"- {link}\n"
            if len(data.get('links', [])) > 50:
                md += f"- ... and {len(data.get('links', [])) - 50} more\n"
